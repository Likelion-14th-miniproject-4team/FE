...
import { startCountdown, completeCountdownItem, getMe } from "../api/api";
...
export default function RouteActive() {
  ...
  const [slackMinutes, setSlackMinutes] = useState(0);
  ...
  useEffect(() => {
    const fetchAlertSetting = async () => {
      try {
        ...
        if (res.alert_enabled && res.alert_offset_minutes) {
          setAlertOffsetMinutes(res.alert_offset_minutes);
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const alertMins = toMinutes(plannedDepartureTime) - alertOffsetMinutes;
    if (nowMins >= alertMins) {
      setAlarmFired(true);
      if (Notification.permission === "granted") {
        new Notification("When2Leave", {
          body: `출발 ${alertOffsetMinutes}분 전이에요! 서둘러 준비하세요.`,
        });
  ...
  }, [now, alertOffsetMinutes, alarmFired, plannedDepartureTime]);
    ...
    try {
      ...
      setSteps((prev) => {
        const updated = prev.map((s, i) => {
          if (s.id === id)
            return { ...s, status: "done", actualDuration: realActual, savedMinutes };
  ...
  const routeSteps = useMemo(
    () => buildRouteSteps(subPaths, plannedDepartureTime),
    [subPaths, plannedDepartureTime],
  );
            ...
            {routeSteps.map((step, index) => (
              <div key={step.id} className="flex items-start gap-3">
                ...
                >
                  ...
                  {step.detail && (
                    <p className="body-xs text-blue-1000 mt-0.5">{step.detail}</p>
                  )}
                </div>
              </div>
            ))}
